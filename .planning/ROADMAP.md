...
### Phase 17: Pub.dev 发布就绪与预编译二进制分发补全

**Goal:** 将 ceres_mpc 打磨到可发布到 pub.dev 的状态，保留 `cargokit` + GitHub Releases 预编译二进制自动下载/验签路径，让普通用户通常无需本地安装 Rust 即可消费
**Requirements**: REL-01, REL-02, REL-03, PKG-01, PKG-02, PKG-03, PKG-04, PKG-05, DOC-01, DOC-02, DOC-03, QA-01, QA-02, QA-03, EX-01
**Depends on:** Phase 16
**Plans:** 2 plans

Plans:
- [x] 17-01-PLAN.md — Package Compliance 与 publish dry-run 清障 (PKG-01, PKG-02, PKG-03, PKG-04, PKG-05, DOC-02, QA-02)
- [x] 17-02-PLAN.md — 预编译分发覆盖与发布文档收口 (REL-01, REL-02, REL-03, DOC-01, DOC-03, QA-01, QA-03, EX-01)

### Phase 18: Resource Safety — FFI Task/Channel Cleanup
**Goal**: 修复 FFI 边界的 task 泄漏、TTL 清理缺陷和 unbounded channel 问题，确保所有异常路径正确释放资源
**Depends on**: Phase 17
**Requirements**: SAFE-01, SAFE-02, SAFE-03, SAFE-04
**Success Criteria** (what must be TRUE):
  1. keygen/sign/recover round==1 中 inject_all 失败时，spawned task 被 abort（不留孤儿 task）
  2. TTL 过期清理路径调用 task_handle.abort() 而非仅 drop
  3. rx_out 使用有界 channel（capacity ≤ 64），替换 unbounded_channel
  4. 所有现有 cargo test --tests 仍通过（零功能回归）
**Plans**: 2 plans
...
- [x] **Phase 16: 批量消息收发优化** - 通过 Notify 信号 + 批量 WireEnvelope 将 ~12 次 HTTP 往返减少到 ~4 次 (completed 2026-04-10)
...
### Phase 16: 批量消息收发优化
**Goal**: 通过 Notify 信号确定性批量收集 + WireEnvelope payloads 批量字段，将 HTTP 往返次数减到尽可能少（每个逻辑轮次一次往返）
**Depends on**: Phase 15
**Requirements**: BATCH-01, BATCH-02, BATCH-03, BATCH-04
**Success Criteria** (what must be TRUE):
  1. 客户端 ChannelRelayConn 在协议 task 等待输入时通过 Notify 信号通知收集方
  2. 客户端 keygen/sign/recover 使用 collect_batch 批量收集 + make_in_progress_batch 批量发送
  3. 服务端对称实现批量收集/注入/响应
  4. WireEnvelope 支持 payloads 批量字段（向后兼容：无 payloads 时走 payload 单条路径）
  5. 现有集成测试（test_dkg/test_dsg/test_rotation/test_backup_export）全部通过
**Plans**: 2 plans

Plans:
- [x] 16-01-PLAN.md — Wave 0 测试 + 客户端 relay.rs Notify + types.rs 批量 WireEnvelope + mpc_engine.rs collect_batch/inject_all 适配 (BATCH-01, BATCH-02, BATCH-04)
- [x] 16-02-PLAN.md — [wave 2, depends 01] 服务端 relay.rs Notify + types.rs 批量 + lib.rs 辅助函数 + handler 适配 (BATCH-01, BATCH-03)
