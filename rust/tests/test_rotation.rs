...
async fn test_session_ttl_eviction() {
    use ceres_mpc::api::mpc_engine::recover;
    ...
    // Call recover — TTL check must fire and return error
    let result = recover(session_id.to_string(), 2, dummy_env_json, None, None);
