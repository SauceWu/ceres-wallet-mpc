...
sequenceDiagram
    ...
    F->>F: load localEncryptedShare from secure storage
    F->>S: exportKey(mpcKeyId) [requires strong auth]
    S->>S: verify identity (MFA, biometrics, etc.)
    S-->>F: serverSharePrivate (Party1's private x1)

    F->>R: exportPrivateKey(localEncryptedShare, serverSharePrivate)
    R->>R: deserialize Party2Private (x2) from localEncryptedShare
    R->>R: deserialize Party1Private (x1) from serverSharePrivate
    R->>R: fullPrivateKey = x1 * x2 (mod n)
    R->>R: verify: address from fullPrivateKey == keygen address
    R-->>F: ExportResult{privateKeyHex, address, exported: true}

    F->>F: show privateKey to user (import into MetaMask, etc.)
    F->>S: markKeyExported(mpcKeyId)
    S->>S: disable MPC operations for this key
```

**Security notes:**
- 导出后 MPC 密钥对应视为已泄露，不应继续用于 MPC 操作
- 服务端必须在导出前进行强身份验证（MFA、生物识别等）
- 导出操作应有审计日志和速率限制
- 完整私钥仅在客户端内存中短暂存在，不应持久化
