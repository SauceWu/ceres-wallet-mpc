...
If you want a runnable reference implementation instead of building from scratch, start with [SauceWu/ceres-mpc-server-demo](https://github.com/SauceWu/ceres-mpc-server-demo).
...
2. Handle 4 methods: `keygen`, `recovery`, `sign`, `export_key`
3. Run Party1-side DKLs23 protocol (sl-dkls23 or compatible) — 4-round protocol (3 HTTP round-trips with batch optimization)
4. Store `serverKeyshare` (Keyshare) securely — binary format, Base64-encoded
5. Manage ephemeral session state between protocol rounds (~3 HTTP round-trips per operation with batching)
...
```
 User              Client App            ceres_mpc SDK           Your Server
  ...
  |                    |                      |                      |
  |                    |                      |  RPC keygen (R3)     | ← final
  |                    |                      |--------------------->| Server protocol done
  |                    |                      |  {batch R3}          | Keyshare pre-persisted
  |                    |                      |<---------------------|
  |                    |                      |                      |
  |                    |  KeygenResult         |                      |
  |                    |<---------------------|                      |
...
> **Note:** The DKLs23 protocol has 4 internal rounds, but batch optimization compresses this to 3 HTTP round-trips. The server pre-persists the Keyshare on the final batch (round 3) since the client may complete without sending another request.
...
```
 User              Client App            ceres_mpc SDK           Your Server
  ...
  |                    |                      | [Rust] recover()     |
  |                    |                      |                      |
  |                    |                      |  RPC recovery (R2-4) | ← Rounds 2,3,4
  |                    |                      |--------------------->|  (3 round-trips,
...
A single method for the DKG protocol. `round=1` creates the session; `round>1` advances it. With batch optimization, completes in 3 HTTP round-trips.

**params (round 1 — create session):**
```json
{}
```
...
// collect_batch: wait for Notify signal, drain all round messages
// Wrap batch in WireEnvelope with payloads array, return
```

**params (rounds 2-4 — advance session):**
```json
{
  "sessionId": "64-char-hex-session-id",
  "round": 2,
...
> **Batch optimization:** Each round batches all protocol messages (ASK + broadcast + P2P) into the `payloads` array. The `payload` field is empty when `payloads` is present. For backward compatibility, if `payloads` is absent, fall back to reading the single `payload` field.
...
**Server-side logic (rounds 2-4):**
```rust
// Decode client batch: WireEnvelope.decode_all_payloads() → Vec<Vec<u8>>
// inject_all: send each message into Relay channel
// collect_batch: wait for Notify signal, drain all round messages
// If Relay closes → protocol complete, extract Keyshare
let keyshare_b64 = base64::encode(keyshare.as_slice());
// Persist keyshare_b64, return completed result with batch WireEnvelope
```
...
A single method for the key refresh protocol. `round=1` creates the session; `round>1` advances it. Same batch WireEnvelope format and ~3 HTTP round-trips as `keygen`.
...
A single method for the DSG protocol. `round=1` creates the session; `round>1` advances it. Same batch WireEnvelope format and ~3 HTTP round-trips as `keygen`, with `protocol: "dsg"`.
